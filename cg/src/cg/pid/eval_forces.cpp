...
void eval_forces::fast_iter(int idx) const {
  //  if (bundle.orig_dist() - *total_disp > cutoff)
  //    return;
...
void eval_forces::fast_vect_iter(int lane_idx) const {
  ...
  *V += horizontal_add(V_);
  ...
  auto i1p_ = to_array<int, N, W>(i1p), i1_ = to_array<int, N, W>(i1),
       i1n_ = to_array<int, N, W>(i1n), i2p_ = to_array<int, N, W>(i2p),
       i2_ = to_array<int, N, W>(i2), i2n_ = to_array<int, N, W>(i2n);

  for (int idx = 0; idx < (int)N; ++idx) {
    if (!mask[idx])
      continue;

    F[i1p_[idx]] -= diff_F_i1p[idx];
    F[i1_[idx]] -= diff_F_i1[idx];
    F[i1n_[idx]] -= diff_F_i1n[idx];
    F[i2p_[idx]] -= diff_F_i2p[idx];
    F[i2_[idx]] -= diff_F_i2[idx];
    F[i2n_[idx]] -= diff_F_i2n[idx];
  }
}
...
void eval_forces::fast_version_t::divide(
  ...
  for (int vfrom = 0; vfrom < fast_vect_n; vfrom += vect_size_hint) {
    ...
    fast_vect_slices.emplace_back(super, vfrom, vto);
  }

  for (int from = fast_vect_n * V; from < n; from += size_hint) {
...
eval_forces::fast_version_t::fast_slice::fast_slice(
    const cg::pid::eval_forces *eval, int from, int to)
    : eval{eval}, from{from}, to{to} {}

void eval_forces::fast_version_t::fast_slice::run() const {
  for (int idx = from; idx < to; ++idx)
    eval->fast_iter(idx);
}
