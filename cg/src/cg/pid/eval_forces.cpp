...
template <typename E>
void eval_forces::iter(const bundle_expr<E> &bundle) const {
  //  if (bundle.orig_dist() - *total_disp > cutoff)
  //    return;
...
void eval_forces::operator()() const {
  for (int idx = 0; idx < *fast_iter_end; ++idx)
    fast_iter(bundles->at(idx));
  for (int idx = *fast_iter_end; idx < bundles->size(); ++idx)
    iter(bundles->at(idx));
}
...
#undef USE_VECTORIZED_IMPLS
#define USE_VECTORIZED_IMPLS 0
...
#else
  int idx = from;
  for (; idx < to && idx < *fast_iter_end; ++idx)
    fast_iter(bundles->at(idx));
  for (; idx < to; ++idx)
    iter(bundles->at(idx));
