#include "features/nl/legacy_update.h"
...
void legacy_update::operator()() const {
  ...
  for (int i1 = 0; i1 < num_particles; ++i1) {
    auto r1 = r->at(i1);
    auto chain1 = chain_idx->at(i1), seq1 = seq_idx->at(i1);

    for (int i2 = i1 + 1; i2 < num_particles; ++i2) {
      auto r2 = r->at(i2);
      auto chain2 = chain_idx->at(i2), seq2 = seq_idx->at(i2);
      ...
      if (orig_dist < req_r) {
        ...
        while (nat_cont_idx < all_nat_cont->size()) {
          auto cur_nat_cont = all_nat_cont->at(nat_cont_idx);
