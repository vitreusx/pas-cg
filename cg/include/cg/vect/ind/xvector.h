...
using xvector =
    typename _xvector_impl<is_indexed_v<T>, T, Alloc, CudaAlloc>::type;

template <typename T, typename Alloc, typename CudaAlloc>
struct _xvector_impl<false, T, Alloc, CudaAlloc> {
  using type = def::xvector<T, Alloc, CudaAlloc>;
};
...
template <typename T, typename Alloc, typename CudaAlloc>
struct _xvector_impl<true, T, Alloc, CudaAlloc> {
  ...
  struct _1<ind_seq<Idxes...>> {
    ...
    struct _2<type_list<Types...>> {
      ...
      template <std::size_t I>
      using _xvector = xvector<_subtype<I>, _alloc<I>, _cuda_alloc<I>>;

      using Base = tuple<_xvector<Idxes>...>;

      class impl : public Base {
      public:
        impl() : Base{_xvector<Idxes>()...} {}

        explicit impl(Alloc alloc, CudaAlloc cuda_alloc)
            : Base{_xvector<Idxes>(alloc.template get<Idxes>(),
                                   cuda_alloc.template get<Idxes>())...} {}

        explicit impl(int n, T const &init = T(), Alloc alloc = Alloc(),
                      CudaAlloc cuda_alloc = CudaAlloc())
            : Base{_xvector<Idxes>(n, init.template get<Idxes>(),
        ...
        explicit impl(int n, E const &e, Alloc alloc = Alloc(),
                      CudaAlloc cuda_alloc = CudaAlloc())
            : Base{_xvector<Idxes>(n, e.template get<Idxes>(),
                                   alloc.template get<Idxes>(),
                                   cuda_alloc.template get<Idxes>())...} {}
