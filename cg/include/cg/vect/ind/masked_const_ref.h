...
struct _masked_const_ref_impl;

template <typename T, typename Idxes, typename Mask>
using masked_const_ref =
    typename _masked_const_ref_impl<is_indexed_v<T>, T, Idxes, Mask>::type;

template <typename T, typename Idxes, typename Mask>
struct _masked_const_ref_impl<false, T, Idxes, Mask> {
  using type = def::masked_const_ref<T, Idxes, Mask>;
};
...
struct _masked_const_ref_impl<true, T, Idx, Mask> {
  ...
  struct _1<ind_seq<_Idxes...>> {
    ...
    struct _2<type_list<Types...>> {
      class impl : public expr_t<T, impl>,
                   public tuple<masked_const_ref<Types, Idx, Mask>...> {
      public:
        using Base = tuple<masked_const_ref<Types, Idx, Mask>...>;
        using Base::Base;
        using Base::get;
        using Base::operator=;
