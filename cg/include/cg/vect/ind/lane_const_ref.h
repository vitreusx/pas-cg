...
struct _lane_const_ref_impl;

template <typename T, std::size_t N, std::size_t W>
using lane_const_ref =
    typename _lane_const_ref_impl<is_indexed_v<T>, T, N, W>::type;

template <typename T, std::size_t N, std::size_t W>
struct _lane_const_ref_impl<false, T, N, W> {
  using type = def::lane_const_ref<T, N, W>;
};
...
template <typename T, std::size_t N, std::size_t W>
struct _lane_const_ref_impl<true, T, N, W> {
  ...
  struct _1<ind_seq<_Idxes...>> {
    ...
    struct _2<type_list<Types...>> {
      class impl : public expr_t<T, impl>,
                   public tuple<lane_const_ref<Types, N, W>...> {
      public:
        using tuple<lane_const_ref<Types, N, W>...>::tuple;
        using tuple<lane_const_ref<Types, N, W>...>::get;
        using tuple<lane_const_ref<Types, N, W>...>::operator=;
