...
namespace nitro {
template <>
struct is_indexed_impl<cg::qa::sync_data> : public std::true_type {};

template <typename E> struct expr_impl<E, cg::qa::sync_data> {
  using type = cg::qa::sync_data_expr<E>;
};

template <typename E> struct auto_expr_impl<E, cg::qa::sync_data> {
  using type = cg::qa::sync_data_auto_expr<E>;
};
}; // namespace nitro