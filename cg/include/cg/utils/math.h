...
template <typename T> auto apx_rsqrt(T const &x) {
  return (T)1 / sqrt(x);
}

template <typename T> auto apx_sqrt(T const &x) {
...
template <typename T> auto nearbyint(T x) {
...
template <typename T> auto isfinite(T const &x) {
