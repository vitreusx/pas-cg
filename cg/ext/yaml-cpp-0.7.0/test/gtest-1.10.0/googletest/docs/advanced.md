...
that can be used in predicate assertion macros (e.g. `EXPECT_PRED_FORMAT2`, etc)
.
...
`type1 and type2 are not the same type` and most likely (depending on the
compiler)
...
The report format conforms to the following JSON Row:
